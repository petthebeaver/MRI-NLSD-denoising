...
if 1
    ...
    if strcmp(denMethod,'bmXd')||strcmp(denMethod,'bm3d')||strcmp(denMethod,'bm4d')
        ...
        if size(fz,3)>1
            addpath ./bm4d
            % NEW SYNTAX FOR RECENT BM4D.M SCRIPT
            % normal profile
            % D=bm4d(fz, 'Gauss', sigmafz,'np');  % bm4d(z, distribution, sigma, profile, do_wiener, verbose)
            % modified profile (slightly higher psnr, slightly smoother estimate)
            D=bm4d(fz, 'Gauss', sigmafz,'mp');  % bm4d(z, distribution, sigma, profile, do_wiener, verbose)
            
            % OLD SYNTAX FOR EARLIER BM4D.M SCRIPT
